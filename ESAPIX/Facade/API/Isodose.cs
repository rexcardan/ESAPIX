    ...
    public class Isodose : ESAPIX.Facade.API.SerializableObject
    {
        ...
        public ESAPIX.Facade.Types.DoseValue Level
        {
            get
            {
                ...
                var local = this;
                return X.Instance.CurrentContext.GetValue<ESAPIX.Facade.Types.DoseValue>((sc) => { if (DefaultHelper.IsDefault(local._client.Level)) { return default(ESAPIX.Facade.Types.DoseValue); } else { return new ESAPIX.Facade.Types.DoseValue(local._client.Level); } });
            }
            set
            {
